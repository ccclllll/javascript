/**
 * element:host element (id选择器或者element节点)
 * options: 配置选项
 */
function ImageEditor(element, options) {
  ...
  if (!ImageEditor.prototype.create) {
    ...
    ImageEditor.prototype.create = function () {
      ...
      canvas.height = parseInt(ImageEditor.getStyle(this.container, 'height')) - parseInt(ImageEditor.getStyle(operate, 'height')) - 24;
      canvas.width = parseInt(ImageEditor.getStyle(this.container, 'width'));
      ...
      reads.onload = (e) => {
        ...
        img.onload = (e) => {
          ...
          eImage.draw(this.canvas, () => {
            ...
            this.imageListContainer.appendChild(canvas);
            eImage.draw(canvas)
            this.imgList.push({
              canvas,
              eImage
    ...
    // 对下方图片进行重绘
    ImageEditor.prototype.updateList = function () {
      for (var i = 0; i < this.imgList.length; i++) {
        if (this.imgList[i].eImage === this.currentEImage) {
          this.currentEImage.draw(this.imgList[i].canvas);
          break;
        }
      }
    }
