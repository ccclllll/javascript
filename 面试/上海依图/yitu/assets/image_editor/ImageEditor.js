 ...
 * options: 配置选项 options.color:笔触颜色和填充颜色
 */
function ImageEditor(element, options) {
  ...
  // 判断是否需要定义ImageEditor.prototype的属性
  if (!ImageEditor.prototype.create) {
    ...
    ImageEditor.prototype._bindEvent = function () {
      this.container.addEventListener('click', (e) => {
        ...
        if (element.className === 'operate-item' || element.parentNode.className === 'operate-item' ||
          ...
          if (OPERATE_MAP[text.trim()]) {
            OPERATE_MAP[text.trim()]();
            if (element.className === 'operate-item' || element.parentNode.className === 'operate-item') {
    ...
    ImageEditor.prototype._buildMenu = function (menu) {
      ...
      for (var index = 0; index < MENUS.length; index++) {
        ...
        text.innerText = MENUS[index].title;
        if (MENUS[index].name === 'addImage') {
   ...
   * @param {*} iWidth 图片宽度
   */
  function scaleImage(cHeight, cWidth, iHeight, iWidth) {
    var height, width;
  ...
  EImage.prototype.draw = function (canvas, callback) {
    ...
    var scale = scaleImage(canvas.height, canvas.width, this.img.height, this.img.width);
    canvas.height = scale.height;
    canvas.width = scale.width;
    var cxt = canvas.getContext('2d');
    cxt.clearRect(0, 0, canvas.width, canvas.height);
    cxt.drawImage(this.img, 0, 0, scale.width, scale.height);
    // 将所有图形画出来
  ...
  function Rect(x, y, width, height, color) {
    Shap.call(this, x, y, color);
    this.height = height;
    this.width = width;
  }

  Rect.prototype.draw = function (ctx) {
    this.color && (ctx.strokeStyle = this.color);
  ...
  function CircleMode(editor) {
    ...
    let mouseup = (e) => {
      if (!editor.currentEImage) {
        return;
      }
      // 根据滑动距离决定圆的半径
      radius = Math.sqrt((endX - beginX) * (endX - beginX) +
        (endY - beginY) * (endY - beginY));
      editor.currentEImage.addShap(editor.canvas, new Circle(beginX, beginY, options.color, radius, false));
    }
    this.bind = function () {
      //console.log(this)
  ...
  function PointMode(editor) {
    let mousedown = (e) => {
      ...
      beginX = e.clientX - editor.position.x;
      beginY = e.clientY - editor.position.y;
      editor.currentEImage && (editor.currentEImage.addShap(editor.canvas, new Circle(beginX, beginY, options.color, 4, true)));
    }
    this.bind = function () {
      editor.canvas.addEventListener('mousedown', mousedown);
    };
  ...
  function RectMode(editor) {
    ...
    let mouseup = (e) => {
      ...
      if (beginX < endX && beginY < endY) {
        editor.currentEImage.addShap(editor.canvas, new Rect(beginX, beginY, endX - beginX, endY - beginY, options.color))
      }

      if (beginX > endX && beginY > endY) {
        editor.currentEImage.addShap(editor.canvas, new Rect(endX, endY, beginX - endX, beginY - endY, options.color))
      }

      if (beginX < endX && beginY > endY) {
        editor.currentEImage.addShap(editor.canvas, new Rect(beginX, endY, endX - beginX, beginY - endY, options.color))
      }

      if (beginX > endX && beginY < endY) {
        editor.currentEImage.addShap(editor.canvas, new Rect(endX, beginY, beginX - endX, endY - beginY, options.color))
      }
  ...
  function LineMode(editor) {
    ...
    let mouseup = (e) => {
      ...
      editor.currentEImage.addShap(editor.canvas, new Line(this.beginX, this.beginY, this.endX, this.endY, options.color))
      this.endX = this.beginX;
      this.endY = this.beginY;
    }
    this.bind = function () {
